...
// Get 获取证书内容，文件不存在时从 cert_versions 兜底恢复。
func (c *DBCache) Get(domain string) (*cert.CachedCert, bool) {
	if cached, ok := c.getFromFile(domain); ok {
		return cached, true
	}
	return c.getFromDB(domain)
}
...
func (c *DBCache) getFromDB(domain string) (*cert.CachedCert, bool) {
	c.mu.Lock()
	defer c.mu.Unlock()

	version, ok := c.certRepo.GetLatestVersionContent(domain)
	if !ok {
		logger.Log.Warn("证书缓存和版本内容均不存在", "domain", domain)
		return nil, false
	}

	cached, err := cachedCertFromPEM(domain, string(version.CertPEM), string(version.PrivateKeyPEM))
	if err != nil {
		logger.Log.Warn("数据库证书版本内容无效", "domain", domain, "error", err)
		return nil, false
	}

	if err := c.writeFilesLocked(domain, cached.CertPEM, cached.KeyPEM); err != nil {
		logger.Log.Warn("从数据库恢复证书文件失败", "domain", domain, "error", err)
	} else {
		logger.Log.Info("已从数据库恢复证书文件缓存", "domain", domain)
	}
	return cached, true
}
...
func (c *DBCache) Put(domain, certPEM, keyPEM string, notBefore, notAfter int64) error {
	...
	if err := c.writeFilesLocked(domain, certPEM, keyPEM); err != nil {
		return err
	}
...
func (c *DBCache) writeFilesLocked(domain, certPEM, keyPEM string) error {
...
func cachedCertFromPEM(domain, certPEM, keyPEM string) (*cert.CachedCert, error) {
	...
	now := time.Now().Unix()
	if parsed.NotAfter.Unix() <= now+config.CacheExpiryBuffer {
		return nil, fmt.Errorf("证书已过期或即将过期")
	}
	return &cert.CachedCert{
		Domain:    domain,
		CertPEM:   certPEM,
